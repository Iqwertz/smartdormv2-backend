...
4.  [**Authentication & Permissions**](./authentication_permissions.md): How users are authenticated against LDAP, how access to every API endpoint is declared and enforced (access rules, startup check, tests), and how to try each role with the dev accounts. **Read this before adding or changing an endpoint.**
